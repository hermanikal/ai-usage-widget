// Claude Pro-only Scriptable widget. Replace placeholders only in your private copy.
const CONFIG = { baseUrl: "https://YOUR-PRIVATE-HOST", apiKey: "PASTE-YOUR-API-KEY", refreshMinutes: 30 };
const COLORS = { bg: new Color("#1a1a1a"), muted: new Color("#999999"), brand: new Color("#DA7756") };
...
async function load() { const fm=FileManager.local(),path=fm.joinPath(fm.documentsDirectory(),"claude_usage_widget_cache.json");try{const r=new Request(CONFIG.baseUrl+"/api/claude-usage");r.headers={"X-API-Key":CONFIG.apiKey};const d=await r.loadJSON();fm.writeString(path,JSON.stringify(d));return d;}catch(e){if(fm.fileExists(path))return JSON.parse(fm.readString(path));throw e;} }

const widget=new ListWidget();widget.backgroundColor=COLORS.bg;widget.setPadding(12,14,12,14);
try {
  const data=await load(),session=data.session||{},weekly=data.weekly||{},projection=data.projection||{};
  const h=widget.addStack();h.layoutHorizontally();h.centerAlignContent();const i=h.addImage(dot());i.imageSize=new Size(12,12);h.addSpacer(6);const title=h.addText("Claude Pro");title.font=Font.boldSystemFont(15);title.textColor=Color.white();
  widget.addSpacer(8);row(widget,"5 hour",session.pct_actual,150);widget.addSpacer(2);detail(widget,`5h reset: ${sessionReset(session.resets_at||session.reset_at)}`);
  widget.addSpacer(6);row(widget,"Weekly",weekly.pct_actual,150);widget.addSpacer(6);
  const projected=projection.projected_pct_at_reset;detail(widget,`Reset ${formatTime(weekly.resets_at||weekly.reset_at||projection.reset_at)}${typeof projected==="number"?` · ~${projected}% weekly`:""}`);
...
} catch(e) { const t=widget.addText("Claude Usage unavailable");t.textColor=Color.white(); }
